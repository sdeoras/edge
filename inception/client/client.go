...
func main() {
	...
	c := inception.NewInceptionClient(conn)
	...
	for {
		if len(b[n:]) < CHUNK {
			if err := stream.Send(&inception.Data{Data: b[n:], Tag: *tag}); err != nil {
				log.Fatal(err)
			}
			break
		} else {
			if err := stream.Send(&inception.Data{Data: b[n : n+CHUNK], Tag: *tag}); err != nil {
				log.Fatal(err)
			}
			n += CHUNK
		}
